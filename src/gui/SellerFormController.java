...
public class SellerFormController implements Initializable {
	...
	public void onBtSaveAction(ActionEvent event) {
		...
		try {
			...
			setErrorMessages(e.getErrors()); // toda a programação dos erros la embaixo estão em função de clicar no
												// botão save
		} catch (DbException e) {
	...
	public void updateFormData() { // aqui que são pego os valores de update para serem impressos
		if (entity == null) {
			...
			if (entity.getBirthDate() != null) {
				dpBirthDate.setValue(LocalDate.ofInstant(entity.getBirthDate().toInstant(), ZoneId.systemDefault())); // aplicação
																														// de
																														// fuso
																														// horario
			}
		...
		if(entity.getDepartment() == null) {
			comboBoxDepartment.getSelectionModel().selectFirst(); // caso o departamento não exista ele vai pegar o primeiro elemento para criar um novo departamento
		}
		else {
			comboBoxDepartment.setValue(entity.getDepartment()); // o Departamento que estiver associado com o vendedor vai para a coombox, caso ele não seja nulo
		}
	}
	...
	private void initializeComboBoxDepartment() { // método para chamada da combo box
